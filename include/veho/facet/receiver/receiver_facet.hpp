...
namespace veho {
    namespace facet {
            ...
            namespace detail {
                ...
                struct update_config_after_adding_listener<Config, Matcher, Callback, false> {
                    constexpr update_config_after_adding_listener(Config&& config, Matcher&& matcher,
                            ...
                            make_receiver_capability_data().push_back(std::forward<Matcher>(matcher),
                                                                      std::forward<Callback>(callback)))) {}

                    using updated_config_type = decltype(
                            std::declval<Config>().template add_capability_data<veho::controller::receive_capability>(
                                    make_receiver_capability_data().push_back(std::declval<Matcher>(), std::declval<Callback>())
                            )
                    );
        ...
        public:
            template <typename Matcher, typename Callback>
            constexpr veho::bus_builder<typename config_updater<Matcher, Callback>::updated_config_type>
            on(Matcher&& matcher, Callback&& callback) {
                return (receiver::detail::ensure_transmitters_do_not_occupy_all_available_mailboxes<Config>(),
                        veho::bus_builder<typename config_updater<Matcher, Callback>::updated_config_type>(
                                config_updater<Matcher, Callback>(
                                        std::move(
                                                const_cast<veho::bus_builder<Config>*>(static_cast<const veho::bus_builder<Config>*>(this))->config),
                                        std::forward<Matcher>(matcher),
                                        std::forward<Callback>(callback)).updated_config));
            }
        };
    }
}
